 ...
 */
const createWidget = async comicData => {
  ...
  if (isDarkMode) {
    listWidget.backgroundGradient = newLinearGradient('#010c1ee6', '#001e38b3');
  } else {
    listWidget.backgroundGradient = newLinearGradient('#b00a0fe6', '#b00a0fb3');
  }

  // add widget title
  const titleTxt = listWidget.addText(comicData.safe_title);
  ...
  // add comic to widget
  const img = await downloadImage(comicData.img);
  const pic = listWidget.addImage(img);
  ...
  if (SHOW_ALT) {
    const subTxt = listWidget.addText(comicData.alt);
